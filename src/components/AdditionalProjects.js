...
const AdditionalProjects = ({ children }) => (
  <StaticQuery
    query={graphql`
      query {
        allFile(filter: {extension: {regex: "/jpg/"}, relativeDirectory: {eq: "additionalProjects"}}) {
          edges {
            node {
              childImageSharp {
    ...
    render={data => (
      <section className={AdditionalProjectStyles.section}>
        <h2 className={HeadersStyles.regular}>Additional Projects</h2>
        <Slider {...settings}>
